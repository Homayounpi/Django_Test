...
import ast
from tracking.mixins import BaseLogginMixin
...
class TestLogginMixin(APITestCase):
    ...
    def test_log_ip_remote_v4_with_port(self):
        request = APIRequestFactory().get('/logging/')
        request.META['REMOTE_ADDR'] = '127.0.0.9: 7000'
        MockLoggingView.as_view()(request).render()
        log = APIRequestLog.objects.first()
        self.assertEqual(log.remote_addr,'127.0.0.9')
    ...
    def test_log_ip_remote_v6(self):
        request = APIRequestFactory().get('/logging/')
        request.META['REMOTE_ADDR'] = '2001:odb8:85a3:0000:0000:8a2e:0370:7334'
        MockLoggingView.as_view()(request).render()
        log = APIRequestLog.objects.first()
        self.assertEqual(log.remote_addr,'2001:odb8:85a3:0000:0000:8a2e:0370:7334')
    ...
    def test_log_params_cleaned_from_personal_list(self):
        ...
        self.assertEqual(ast.literal_eval(log.query_params),{
            'api':BaseLogginMixin.CLEANED_SUBSTITUTE,
            'capitalized':'12345',
            'my_field':BaseLogginMixin.CLEANED_SUBSTITUTE,
        })
    ...
    @mock.patch('tracking.models.APItracking')
    def test_log_doesnt_prevent_api_call_if_bog_save_fails(self,mock_save):
