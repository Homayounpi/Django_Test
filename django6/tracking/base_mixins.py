...
class BaseLogginMixin:
    ...
    def finalize_response(self,request,response,*args,**kwargs):
        ...
        if self.should_log(request,response):
            ...
            try:
                ...
                logger.exception('Logging API call raise exceptions')
    ...
    def _clean_data(self,data):
        ...
        if isinstance(data,dict):
            SENSITIVE_FIELDS = {'api','token','key','password','secret','signature'}
            if self.sensitive_fields:
                SENSITIVE_FIELDS = SENSITIVE_FIELDS | {fields.lower() for fields in self.sensitive_fields}
